...
def _llm(model: str, system: str, user: str, temperature: float = 0.2) -> str:
    ...
    headers = {"Authorization": f"Bearer {
        Config.OPENROUTER_API_KEY}", "Content-Type": "application/json"}
    payload = {"model": model, "messages": [{"role": "system", "content": system}, {
        "role": "user", "content": user}], "temperature": temperature}
    r = requests.post(Config.OPENROUTER_URL, headers=headers,
...
def summarize_paper(title: str, abstract: str, results_text: str) -> str:
    ...
    return _llm("openai/gpt-4o-mini", system, prompt)


def extract_entities_triples(text: str) -> str:
    system = ("Extract entities (organism, tissue/system, intervention, outcome, measurement) "
